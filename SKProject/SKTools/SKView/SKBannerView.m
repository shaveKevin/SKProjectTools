...
#import "SKBannerView.h"

static NSInteger const kStartTag = 1000;
static NSInteger const kDefaultScrollInterval = 2;

@interface SKBannerView() <UIScrollViewDelegate>
...
- (void)initWithImageURLs:(NSArray *)imageURLs placeholder:(UIImage *)placeholder delegate:(id<SKBannerViewDelegate>)delegate
{
    [self initWithCount:imageURLs.count delegate:delegate edgeInsets:UIEdgeInsetsZero];
}


- (void)initWithImageURLs:(NSArray *)imageURLs placeholder:(UIImage *)placeholder delegate:(id<SKBannerViewDelegate>)delegate edgeInsets:(UIEdgeInsets)edgeInsets
{
...
{
    self.count = count;
    self.skBannerViewDelegate = delegate;
    ...
    for (int i = 0; i < count; i++) {
        ...
        [self.skBannerViewDelegate imagePlayerView:self loadImageForImageView:imageView index:i];
...
- (void)handleTapGesture:(UIGestureRecognizer *)tapGesture
{
    ...
    if (self.skBannerViewDelegate && [self.skBannerViewDelegate respondsToSelector:@selector(imagePlayerView:didTapAtIndex:)]) {
        [self.skBannerViewDelegate imagePlayerView:self didTapAtIndex:index];
    }
}
